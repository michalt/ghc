...
findPtrTys i ty
  ...
  | otherwise
  = case repType ty of
      RepType rep_ty | typePrimRep rep_ty == PtrRep -> return (i + 1, [(i, ty)])
                     | otherwise                    -> return (i,     [])
      UbxTupleRepType rep_tys  -> foldM (\(i, extras) rep_ty -> if typePrimRep rep_ty == PtrRep
...
getDataConArgTys dc con_app_ty
  = do { (_, ex_tys, _) <- instTyVars ex_tvs
       ; let RepType rep_con_app_ty = repType con_app_ty
