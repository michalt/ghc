...
collect (_, e) = go [] e
  where
    ...
    go xs (AnnLam x (_,e)) 
      | UbxTupleRepType _ <- repType (idType x)
...
schemeE d s p (AnnCase scrut _ _ [(DataAlt dc, [bind1, bind2], rhs)])
   | isUnboxedTupleCon dc, RepType rep_ty <- repType (idType bind1), VoidRep <- typePrimRep rep_ty
        -- Convert
   ...
   | isUnboxedTupleCon dc, RepType rep_ty <- repType (idType bind2), VoidRep <- typePrimRep rep_ty
   = --trace "automagic mashing of case alts (# a, VoidArg #)" $
     doCase d s p scrut bind1 [(DEFAULT, [], rhs)] True{-unboxed tuple-}

schemeE d s p (AnnCase scrut _ _ [(DataAlt dc, [bind1], rhs)])
   | isUnboxedTupleCon dc, RepType _ <- repType (idType bind1)
        -- Similarly, convert
        --      case .... of x { (# a #) -> ... }
        -- to
        --      case .... of a { DEFAULT -> ... }
...
doCase d s p (_,scrut) bndr alts is_unboxed_tuple
  | UbxTupleRepType _ <- repType (idType bndr)
  = unboxedTupleException
  | otherwise
  = let
        ...
        codeAlt alt@(_, bndrs, (_,rhs))
           ...
           | any (\bndr -> case repType (idType bndr) of UbxTupleRepType _ -> True; _ -> False) bndrs
           = unboxedTupleException
...
generateCCall d0 s p (CCallSpec target cconv safety) fn args_r_to_l
   = let
         ...
         pargs _ [] = return []
         pargs d (a:az)
            = let RepType arg_ty = repType (exprType (deAnnotate' a))
...
maybe_getCCallReturnRep fn_ty
   = let (_a_tys, r_ty) = splitFunTys (dropForAlls fn_ty)
         ...
         r_reps = case repType r_ty of
                      UbxTupleRepType reps -> map typePrimRep reps
                      RepType _            -> blargh
