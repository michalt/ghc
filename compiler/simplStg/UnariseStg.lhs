...
unarise us binds = zipWith (\us -> unariseBinding us emptyVarEnv) (listSplitUniqSupply us) binds
...
unariseExpr us rho e = case e of
  StgApp f [] | UbxTupleRepType tys <- repType (idType f)
  ...
  StgCase e case_lives alts_lives bndr srt alt_ty alts
    ...
    where (us1, us2) = splitUniqSupply us
          (alt_ty', alts') = case repType (idType bndr) of
                    UbxTupleRepType tys -> case alts of 
                    ...
                    RepType _ -> (alt_ty, zipWith (\us alt -> unariseAlt us rho alt) (listSplitUniqSupply us2) alts)
...
unariseIdBinder us rho x = case repType (idType x) of
    RepType _           -> (us, rho, [x])
    UbxTupleRepType tys -> let (us0, us1) = splitUniqSupply us
